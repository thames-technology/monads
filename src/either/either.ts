...
export interface LeftEither<L, R> extends Either<L, R> {
  unwrapLeft(): L;
  unwrapRight(): never;
  left(): SomeOption<L>;
  right(): NoneOption<R>;
...
export interface RightEither<L, R> extends Either<L, R> {
  unwrapLeft(): never;
  unwrapRight(): R;
  left(): NoneOption<L>;
  right(): SomeOption<R>;
