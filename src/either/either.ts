...
export function isEither<L, R>(val: Either<L, R> | any): val is Either<L, R> {
  return (
    isEqual(val.type, EitherType.Left) || isEqual(val.type, EitherType.Right)
  );
}

export function isLeft<L, R>(val: Either<L, R>): val is ResLeft<L, R> {
  throwIfFalse(isEither(val), "val is not a Either");
  return val.isLeft();
}
